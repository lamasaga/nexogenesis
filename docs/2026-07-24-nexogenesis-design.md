...
约定目录下的 **markdown 是唯一语义事实之源**。任何时候删掉全部代码与可重建索引（图、RAG、数据库等），只留 markdown，知识内容零损失。

图谱、向量索引、关系数据库都是**可重建索引**，永不持有不可替代的信息。**git 是强烈推荐的版本层**（历史、回滚、hook），不是思想本体；未启用 git 时不得声称操作已可逆。
