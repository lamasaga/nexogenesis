...
def construct_cmd(root, diagnose, lens, deep_cards, wave_buffers, plan, apply, auto):
    ...
    # 显式 apply，或 auto+lens 且已有 batch → 落盘
    ...
    if diagnose or lens is None:
        ...
        report = render_diagnose_report(store, buffer_records, signals)
        ...
        if auto:
            ...
            suggested = suggest_lenses(signals)
            ...
            runbook = write_construct_runbook(
                ...
                tmp_dir, suggested=suggested, batch_path=batch_path
    ...
    deep = load_cards_by_ids(store, card_ids)
    ...
    lens_signals = list(signals.get(lens.lower()) or [])
